...
function removeFirstOccurrences(str, value) {
  ...
  return startIndex >= 0 ? str.replace(value, '') : str;
}
...
function countVowels(str) {
  return str.split('').filter((ch) => 'aeiouy'.includes(ch.toLowerCase()))
    .length;
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((s) => (alphabet.indexOf(s) >= 0 ? decode[alphabet.indexOf(s)] : s))
    .join('');
}
